...
export default function FindJobsPage() {
  ...
  const [activeTab, setActiveTab] =
    useState<'all' | 'featured'>('all');

  const [sortBy, setSortBy] =
    useState<'date' | 'title'>('date');
  ...
  const displayedJobs = useMemo(() => {
    ...
    if (activeTab === 'featured') {
      result = result.filter(
        (job) =>
          Boolean(job.salary) ||
          Boolean(job.company?.companyName),
      );
    }
            ...
            <div className="sticky top-24 space-y-5">
              ...
              <div className="rounded-2xl border border-slate-100 bg-white p-7 shadow-sm">
                ...
                <p className="mt-3 text-sm leading-7 text-slate-500">
                  ...
                  resume to increase your chances of getting
              ...
              <div className="rounded-2xl border border-slate-100 bg-white p-7 shadow-sm">
                ...
                <p className="mt-3 text-sm leading-7 text-slate-500">
                  Use specific job titles, locations and
                  employment types to find opportunities that
                  match your goals.
                </p>
...
function JobCard({
  ...
  return (
    <article className="mb-5 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:p-7">
...
/>
      ...
      <div className="mt-7 grid gap-6 md:grid-cols-[1fr_145px]">
        ...
        <div>
          ...
          <Link
            href={`/jobs/${job.id}`}
            className="mt-4 block text-xl font-bold text-slate-950 transition hover:text-[#1671B9] sm:text-2xl"
          >
            {job.title}
          </Link>
